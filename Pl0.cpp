...
int getsym()
{
	//状态1（Start）：起始状态，过滤空白字符
	int i, j, k;
	while (ch == ' ' || ch == 10 || ch == 9)
	{
		getchdo;// 保持在状态1直到遇到非空白字符
	}
	// 状态2：检测到字母，进入标识符/保留字处理
	if (ch >= 'a' && ch <= 'z')
	{
		k = 0;
		do {
			...
			getchdo;//状态2自循环（持续收集字母数字）
		} while (ch >= 'a' && ch <= 'z' || ch >= '0' && ch <= '9');
		//字符串终止符（状态3：标识符/保留字结束）
		a[k] = 0;
		strcpy(id, a);
		i = 0;
		j = norw - 1;
...
int statement(bool* fsys, int* ptx, int lev)
{
	...
	else
	{
		...
		{
			if (sym == writesym) // 写语句			/* 准备按照write语句处理，与read类似*/
			{
				getsymdo;
				if (sym == lparen)
				{
			...
			{
				if (sym == callsym) // 调用语句		/* 准备按照call语句处理*/
				{
					getsymdo;
					if (sym != ident)
					{
				...
				{
					if (sym == ifsym) // IF语句    /*准备按照if语句处理*/ // 语法 
					{ // 碰到终结符匹配，非终结符调用  // 语法是结构 
						getsymdo; // 语法 
						memcpy(nxtlev, fsys, sizeof(bool) * symnum); 
						nxtlev[thensym] = true; 
						nxtlev[dosym] = true;    
						conditiondo(nxtlev, ptx, lev);   // 语法 
						if (sym == thensym) // 语法 
						{
							getsymdo; // 语法 
						}
